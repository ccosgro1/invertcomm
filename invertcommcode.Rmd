---
title: "Jennings Invertrbrate Community"
author: "crc31 Cosgrove"
date: "4 October 2018"
output: html_document
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r combine datasets}
#Combine the four individual CSVs from each sample date
#Note to self: never do in a new tab what you can do in a new column...

#smartbind in gtools can combine datasets
library(gtools)

##Load invert community files and merge them into one csv file.
date4=read.csv("C:/Users/crc31/Dropbox/Jennings Invertebrates/beerbottles_10.15.16.csv")
date4[is.na(date4)]<-0
date4=date4[c(1:93),c(1:129)]
date3=read.csv("C:/Users/crc31/Dropbox/Jennings Invertebrates/beerbottles_8.10.16.csv")
date3[is.na(date3)]<-0
date3=date3[c(1:95),c(1:115)]
date2=read.csv("C:/Users/crc31/Dropbox/Jennings Invertebrates/beerbottles_5.15.16.csv")
date2[is.na(date2)]<-0
date2=date2[c(1:95),c(1:117)]
date1=read.csv("C:/Users/crc31/Dropbox/Jennings Invertebrates/beerbottles_10.6.15.csv")
date1[is.na(date1)]<-0
date1=date1[c(1:91),c(1:80)]
colnames(date1)

commdata=smartbind(date1,date2,date3,date4)
write.csv(commdata)

#Once datasets are combined, check to make sure no misspellings in col names
colnames(commdata)

#remove any NAs
is.na(commdata[,1])
#91,186,281,374 = rows with all NA's
commdata=commdata[-c(91,186,281,374),]
commdata[is.na(commdata)]<-0
is.na(commdata)

#write.csv(commdata,"invertcommdata2.csv")
```

or the EntSA poster:
1. Totals by ecosystem, data
2. Gamma diversity between ecosystems
3. RDA


```{r Load data}
#Load Data
commdata=read.csv("C:/Users/crc31/OneDrive/Documents/School/Research/Data/Inverts/invertcomm/invertcommdata.csv", header=T)
treedata=read.csv("C:/Users/crc31/Dropbox/Jennings 2016/New Adult Basal Area.csv")
soildata=read.csv("C:/Users/crc31/OneDrive/Documents/School/Grad School/SpatialStats/soil data May 08 for tree plots.csv")
flying.ffg=read.csv("C:/Users/crc31/Dropbox/Jennings Invertebrates/flyingFFG.csv")
ordersums=read.csv("C:/Users/crc31/Dropbox/Jennings Invertebrates/invertcommdata_ordersums.csv", header=T)
dim(soildata)
treedata[is.na(treedata)] <- 0
treedata=treedata[treedata$ecosys!="S",] #Remove the stream plots -- Will need to put these back in when I fix the commdata$ecosystem columns.
dim(treedata)
##Tree SPP which are OVER 1% of total basal area: ACERUB, ACESAC, CARCOR, CAROVA, FAGAME, FRAAME, LIRTUP, PLAOCC, POPDEL, PRUSER, QUEALB, QUEPAL, QUERUB, TILAME

##Combine tree data and hellinger transform
tree.ringsum=treedata[1:83,]
for(i in 1:83) {
	tree.ringsum3=treedata[i,10:42]+treedata[i+83,10:42]+treedata[i+166,10:42]  }
for(i in 1:83) {
	tree.ringsum2=treedata[i,10:42]+treedata[i+83,10:42] }
for(i in 1:83) {
	tree.ringsum1=treedata[i,10:42] }
rownames(tree.ringsum)=tree.ringsum[,1]

tree.rowsums=apply(tree.ringsum[10:42],1,sum)
tree.hel=tree.ringsum
tree.hel[,10:42]=tree.hel[,10:42]/tree.rowsums
tree.hel[,10:42]=sqrt(tree.hel[,10:42])

## Load libraries - Do I even need all of these?
library(vegan)
library(nlme)
library(ggplot2)
library(lme4)
library(labdsv)
dim(commdata)

#merge soil and tree data with commdata
commdata=merge(commdata,soildata, by="Plot")
commdata=merge(commdata,tree.hel)

#Separate out dates for future lines of code:
date4=commdata[commdata$Date=="10.15.16",]
date2=commdata[commdata$Date=="5.5.16",]
date3=commdata[commdata$Date=="8.10.16",]
date1=commdata[commdata$Date=="10.6.15",]
```

```{r Rank Abundance}
# Let's look at a rank abundance plot
bug.colsums=apply(ordersums[,5:18],2,sum)
plot(rank(bug.colsums),bug.colsums, xlab="Invert species")

aggregate(ordersums$Psocoptera, by=list(ordersums$Habitat), FUN='sum')

orderrab=ordersums

##separate out by Habitat so that I can figure out within ecosystem rank abundance.
orderrab.bot=orderrab[orderrab$Habitat=="B",]
bug.colsums=apply(orderrab.bot[,5:18],2,sum)
plot(rank(bug.colsums),bug.colsums, xlab="Invert species")

orderrab.dis=orderrab[orderrab$Habitat=="D",]
bug.colsums=apply(orderrab.dis[,5:18],2,sum)
plot(rank(bug.colsums),bug.colsums, xlab="Invert species")

orderrab.rip=orderrab[orderrab$Habitat=="R",]
bug.colsums=apply(orderrab.rip[,5:18],2,sum)
plot(rank(bug.colsums),bug.colsums, xlab="Invert species")

orderrab.upl=orderrab[orderrab$Habitat=="U",]
bug.colsums=apply(orderrab.upl[,5:18],2,sum)
plot(rank(bug.colsums),bug.colsums, xlab="Invert species")
```

```{r Exploratory Stats}
#General summation and graph by date
sum(commdata$Total) #Total=99517 individuals
plot(commdata$Total~commdata$Date) #Most individuals in falls
dateordered=ordered(commdata$Date, levels = c("10.6.15","5.5.16","8.10.16","10.15.16"))
sumplot=boxplot(commdata$Total~dateordered, xlab="Date", ylab="Average Site Sum")
sumplot2=boxplot(commdata$Total~dateordered, xlab="Date", ylab="Average Site Sum", ylim=c(0,1000))



ecoplot=plot(commdata$Total~commdata$Habitat, xlab="Ecosystem", ylab="Average Ecosystem Sum")
ecoplot2=plot(commdata$Total~commdata$Habitat, xlab="Ecosystem", ylab="Average Ecosystem Sum", ylim=c(0,1000))

#abundance by date and ecosystem
abund=aov(commdata$Total~commdata$Habitat*commdata$Date)
summary(abund)
plot(abund)
shapiro.test(commdata$Total)
#shapiro.test is super non-normal; log transform best transforms the datam but not great..
sqtot=lm(commdata$Total)
abund1=aov(sqtot~commdata$Habitat*commdata$Date)
summary(abund1)
plot(abund1)
shapiro.test(sqtot)
TukeyHSD(abund1)


#sum by date and ecosystem
date4=commdata[commdata$Date=="10.15.16",]
sum(date4$Total)
date2=commdata[commdata$Date=="5.5.16",]
sum(date2$Total)
date3=commdata[commdata$Date=="8.10.16",]
sum(date3$Total)
date1=commdata[commdata$Date=="10.6.15",]
sum(date1$Total)



#Rank abundance
totalrs=apply(commdata[,8:158],2,sum)
plot(rank(totalrs),totalrs, xlab="invert species", xlim=c(1,158))
plot(rank(totalrs),totalrs, xlab="invert species", xlim=c(1,158), ylim=c(0,2500))

```

```{r Diversity}
#Gamma diversity between ecosystems
rownames(date1)=date1[,1]
dim(date1)
Hdate1=diversity(date1[1:89,8:158])
raredate1=rarefy(date1[,8:158], 2)-1
specno=specnumber(commdata[8:158], MARGIN=1)

botrich=lme(specno~as.factor(TerrType)*Date+Plot,random=~1, data=commdata)
summary(botrich)
TukeyHSD(botrich)
shapiro.test(specscno)

#Alpha
alpha1=fisher.alpha(date1[,8:158])
alpha2=fisher.alpha(date2[,8:158])
alpha3=fisher.alpha(date3[,8:158])
alpha4=fisher.alpha(date4[,8:158])
boxplot(alpha1)
plot(alpha2)
plot(alpha3)
plot(alpha4)


#Gamma
gammadiv=with(date1, tapply(specnumber(date1[,8:158]),Habitat))
plot(gammadiv)
gamma2=with(date2, tapply(specnumber(date2[,8:158]),Habitat))
gamma3=with(date3, tapply(specnumber(date3[,8:158]),Habitat))
gamma4=with(date4, tapply(specnumber(date4[,8:158]),Habitat))
plot(gammadiv,gamma2,gamma3,gamma4)
#Beta
alpha1 <- with(date1, tapply(specnumber(date1[,8:158]), Habitat, mean))
alpha2 <- with(date2, tapply(specnumber(date2[,8:158]), Habitat, mean))
alpha3 <- with(date3, tapply(specnumber(date3[,8:158]), Habitat, mean))
alpha4 <- with(date4, tapply(specnumber(date4[,8:158]), Habitat, mean))
beta1=gammadiv/alpha1-1
beta2=gamma2/alpha2-1
beta3=gamma3/alpha3-1
beta4=gamma4/alpha4-1

boxplot(alpha1,alpha2,alpha3,alpha4, xlab="Date", ylab="Species Richness")
mtext(side=1,at=1,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=2,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=2,line=1)


##Real stats or something
colnames(date1)
dim(date1)
rownames(date1)=date1[,1]
Hdate1=diversity(date1[8:158], index="shannon")
rownames(date2)=date2[,1]
Hdate2=diversity(date2[8:158], index="shannon")
rownames(date3)=date3[,1]
Hdate3=diversity(date3[8:158], index="shannon")
rownames(date4)=date4[,1]
Hdate4=diversity(date4[8:158], index="shannon")
boxplot(Hdate1, Hdate2, Hdate3, Hdate4, xlab="Date", ylab="Shannon's Index")
mtext(side=1,at=1,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=2,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=2,line=1)


#shannon's diversity by ecosystem type
bot=commdata[commdata$TerrType=="B",]
b1=bot[bot$Date=="10.6.15",]
b2=bot[bot$Date=="5.5.16",]
b3=bot[bot$Date=="8.10.16",]
b4=bot[bot$Date=="10.15.16",]
dim(bot)
Hbot=diversity(bot[8:158], index="shannon")
hb1=diversity(b1[8:158], index="shannon")
hb2=diversity(b2[8:158], index="shannon")
hb3=diversity(b3[8:158], index="shannon")
hb4=diversity(b4[8:158], index="shannon")
boxplot(hb1, hb2, hb3, hb4, xlab="bottomland diversity", ylab="Shannon's Diversity", main="Bottomland")
mtext(side=1,at=1,"6 Oct 2015",las=0,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=0,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=0,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=0,line=1)

##Let's make a nice boxplot in ggplot
ggplot(decompk.row.ripup, aes(x=Species,y=as.numeric(as.character(decompk)),colour=LumpedTrans))+
  geom_boxplot(position=position_dodge(0.9), size=1)+
  #ggtitle("RipUp TerrTypexSpecies")+
  theme_bw()+
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        axis.text=element_text(size=16),
        axis.title=element_text(size=20),
        axis.title.x=element_blank(),
        legend.position = "top",
        axis.ticks.length = unit(-0.25,"cm"),
        aspect.ratio = 0.75
        )+
  scale_color_manual(values=c("darkolivegreen3","cornsilk4","cornflowerblue"))+
  ylab("Decay k")+
  scale_y_continuous(limits=c(-6.5,0))


srb1=specnumber(b1[8:158], MARGIN=1)
srb2=specnumber(b2[8:158], MARGIN=1)
srb3=specnumber(b3[8:158], MARGIN=1)
srb4=specnumber(b4[8:158], MARGIN=1)
boxplot(srb1,srb2,srb3,srb4, xlab="bottomland richness", ylab="species richness")
mtext(side=1,at=1,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=2,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=2,line=1)


rip=commdata[commdata$TerrType=="R",]
r1=rip[rip$Date=="10.6.15",]
r2=rip[rip$Date=="5.5.16",]
r3=rip[rip$Date=="8.10.16",]
r4=rip[rip$Date=="10.15.16",]
Hrip=diversity(rip[8:158], index="shannon")
hr1=diversity(r1[8:158], index="shannon")
hr2=diversity(r2[8:158], index="shannon")
hr3=diversity(r3[8:158], index="shannon")
hr4=diversity(r4[8:158], index="shannon")
boxplot(hr1, hr2, hr3, hr4, xlab="riparian diversity", ylab="shannons diversity", main="Riparian")
mtext(side=1,at=1,"6 Oct 2015",las=0,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=0,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=0,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=0,line=1)


srr1=specnumber(r1[8:158], MARGIN=1)
srr2=specnumber(r2[8:158], MARGIN=1)
srr3=specnumber(r3[8:158], MARGIN=1)
srr4=specnumber(r4[8:158], MARGIN=1)
boxplot(srr1,srr2,srr3,srr4, xlab="riparian richness", ylab="species richness")
mtext(side=1,at=1,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=2,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=2,line=1)


upl=commdata[commdata$TerrType=="U",]
u1=upl[upl$Date=="10.6.15",]
u2=upl[upl$Date=="5.5.16",]
u3=upl[upl$Date=="8.10.16",]
u4=upl[upl$Date=="10.15.16",]
Hupl=diversity(upl[8:158], index="shannon")
hu1=diversity(u1[8:158], index="shannon")
hu2=diversity(u2[8:158], index="shannon")
hu3=diversity(u3[8:158], index="shannon")
hu4=diversity(u4[8:158], index="shannon")
boxplot(Hupl,hu1, hu2, hu3, hu4, xlab="upland diversity", ylab="shannons diversity")
mtext(side=1,at=1,"All Dates",las=2,line=1, srt=45)
mtext(side=1,at=2,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=3,"5 May 2016",las=2,line=1)
mtext(side=1,at=4,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=5,"15 Oct 2016",las=2,line=1)


sru1=specnumber(u1[8:158], MARGIN=1)
sru2=specnumber(u2[8:158], MARGIN=1)
sru3=specnumber(u3[8:158], MARGIN=1)
sru4=specnumber(u4[8:158], MARGIN=1)
boxplot(sru1,sru2,sru3,sru4, xlab="upland richness", ylab="species richness")
mtext(side=1,at=1,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=2,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=2,line=1)


dis=commdata[commdata$TerrType=="D",]
rownames(dis)=dis[,1]
Hdis=diversity(dis[8:158], index="shannon")

boxplot(Hbot, Hrip, Hupl, Hdis, xlab="Ecosystem", ylab="Shannon's Diversity")
mtext(side=1,at=1,"Bottomland",las=2,line=1, srt=45)
mtext(side=1,at=2,"Riparian",las=2,line=1)
mtext(side=1,at=3,"Upland",las=2,line=1)
mtext(side=1,at=4,"Disturbed",las=2,line=1)


halp=aov(Hbot~bot$Date+bot$Plot)
summary(halp)
TukeyHSD(halp)
halp=aov(Hrip~rip$Date+rip$Plot)
summary(halp)
halp=aov(Hupl~upl$Date+upl$Plot)
summary(halp)
halp=aov(Hdis~dis$Date+dis$Plot)
summary(halp)

##Ecotone Diversity and Richness
#uplbot
uplbot=commdata[commdata$TerrType=="UB"|commdata$TerrType=="BU",]
dim(uplbot)
ub1=uplbot[uplbot$Date=="10.6.15",]
ub2=uplbot[uplbot$Date=="5.5.16",]
ub3=uplbot[uplbot$Date=="8.10.16",]
ub4=uplbot[uplbot$Date=="10.15.16",]

ubdall=diversity(uplbot[8:158], index="shannon")
ubd1=diversity(ub1[8:158], index="shannon")
ubd2=diversity(ub2[8:158], index="shannon")
ubd3=diversity(ub3[8:158], index="shannon")
ubd4=diversity(ub4[8:158], index="shannon")
boxplot(ubdall, ubd1, ubd2, ubd3, ubd4, xlab="ub diversity", ylab="shannons diversity")
mtext(side=1,at=1,"All Dates",las=2,line=1, srt=45)
mtext(side=1,at=2,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=3,"5 May 2016",las=2,line=1)
mtext(side=1,at=4,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=5,"15 Oct 2016",las=2,line=1)

ubr1=specnumber(u1[8:158], MARGIN=1)
ubr2=specnumber(u2[8:158], MARGIN=1)
ubr3=specnumber(u3[8:158], MARGIN=1)
ubr4=specnumber(u4[8:158], MARGIN=1)
boxplot(ubr1,ubr2,ubr3,ubr4, xlab="UB spp richness", ylab="species richness")
mtext(side=1,at=1,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=2,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=2,line=1)

#ripbot
ripbot=commdata[commdata$TerrType=="RB"|commdata$TerrType=="BR",]
rb1=ripbot[ripbot$Date=="10.6.15",]
rb2=ripbot[ripbot$Date=="5.5.16",]
rb3=ripbot[ripbot$Date=="8.10.16",]
rb4=ripbot[ripbot$Date=="10.15.16",]

rbdall=diversity(ripbot[8:158], index="shannon")
rbd1=diversity(rb1[8:158], index="shannon")
rbd2=diversity(rb2[8:158], index="shannon")
rbd3=diversity(rb3[8:158], index="shannon")
rbd4=diversity(rb4[8:158], index="shannon")
boxplot(rbdall, rbd1, rbd2, rbd3, rbd4, xlab="RB diversity", ylab="shannons diversity")
mtext(side=1,at=1,"All Dates",las=2,line=1, srt=45)
mtext(side=1,at=2,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=3,"5 May 2016",las=2,line=1)
mtext(side=1,at=4,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=5,"15 Oct 2016",las=2,line=1)

rbr1=specnumber(u1[8:158], MARGIN=1)
rbr2=specnumber(u2[8:158], MARGIN=1)
rbr3=specnumber(u3[8:158], MARGIN=1)
rbr4=specnumber(u4[8:158], MARGIN=1)
boxplot(rbr1,rbr2,rbr3,rbr4, xlab="RB spp richness", ylab="species richness")
mtext(side=1,at=1,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=2,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=2,line=1)

#uplrip
uplrip=commdata[commdata$TerrType=="UR"|commdata$TerrType=="RU",]
ur1=uplrip[uplrip$Date=="10.6.15",]
ur2=uplrip[uplrip$Date=="5.5.16",]
ur3=uplrip[uplrip$Date=="8.10.16",]
ur4=uplrip[uplrip$Date=="10.15.16",]

urdall=diversity(uplrip[8:158], index="shannon")
urd1=diversity(ur1[8:158], index="shannon")
urd2=diversity(ur2[8:158], index="shannon")
urd3=diversity(ur3[8:158], index="shannon")
urd4=diversity(ur4[8:158], index="shannon")
boxplot(urdall,urd1, urd2, urd3, urd4, xlab="ur diversity", ylab="shannons diversity")
mtext(side=1,at=1,"All Dates",las=2,line=1, srt=45)
mtext(side=1,at=2,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=3,"5 May 2016",las=2,line=1)
mtext(side=1,at=4,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=5,"15 Oct 2016",las=2,line=1)


urr1=specnumber(u1[8:158], MARGIN=1)
urr2=specnumber(u2[8:158], MARGIN=1)
urr3=specnumber(u3[8:158], MARGIN=1)
urr4=specnumber(u4[8:158], MARGIN=1)
srripupl=boxplot(urr1, urr2, urr3, urr4, xlab="ur spp richness", ylab="species richness")
mtext(side=1,at=1,"6 Oct 2015",las=2,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=2,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=2,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=2,line=1)

boxplot(hu4, urd4, hr4, xlab="Ecosystem", ylab="Shannons diversity")
mtext(side=1,at=1,"U",las=2,line=1, srt=45)
mtext(side=1,at=2,"UR",las=2,line=1)
mtext(side=1,at=3,"R",las=2,line=1)

boxplot(sru4, urr4, srr4, xlab="Ecosystem", ylab="species richness")
mtext(side=1,at=1,"U",las=2,line=1, srt=45)
mtext(side=1,at=2,"UR",las=2,line=1)
mtext(side=1,at=3,"R",las=2,line=1)



####GRAHING FOR ESA
boxplot(hr1, hr2, hr3, hr4, ylab="Shannon's Diversity")
title(main = "Riparian")

boxplot(srr1,srr2,srr3,srr4, xlab="Date", ylab="Taxonomic Richness", ylim=c(5,25))
mtext(side=1,at=1,"6 Oct 2015",las=0,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=0,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=0,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=0,line=1)

boxplot(urd1, urd2, urd3, urd4, xlab="") #Ecotone Diversity
title(main="Riparian-Upland")


srripupl=boxplot(urr1, urr2, urr3, urr4, xlab="Date") #SR RipUpl
mtext(side=1,at=1,"6 Oct 2015",las=0,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=0,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=0,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=0,line=1)

boxplot(hu1, hu2, hu3, hu4, xlab="") #Upland Diversity
title(main="Upland")

boxplot(sru1,sru2,sru3,sru4, xlab="Date", main="Upland")
mtext(side=1,at=1,"6 Oct 2015",las=0,line=1, srt=45)
mtext(side=1,at=2,"5 May 2016",las=0,line=1)
mtext(side=1,at=3,"10 Aug 2016",las=0,line=1)
mtext(side=1,at=4,"15 Oct 2016",las=0,line=1)



###
commdata.core=commdata[commdata$EP=="Core",]
dim(commdata.core)
halldates=diversity(commdata.core[,8:158], index="shannon")
alldates=lme(halldates~Habitat*Date, random = ~ 1|Plot, data=commdata.core)
anova(alldates)

sralldates=specnumber(commdata.core[8:158], MARGIN=1)
datesall=lme(sralldates~Habitat*Date, random=~1|Plot, data=commdata.core)
anova(datesall)

commdata.edge=commdata[commdata$EP!="Trans",]
commdata.edge=commdata.edge[commdata.edge$Habitat!="UR",]
dim(commdata.edge)

halldates1=diversity(commdata.edge[8:158], index="shannon")
alldates1=lme(halldates1~Habitat*Date, random = ~ 1|Plot, data=commdata.edge)
anova(alldates1)

sralldates1=specnumber(commdata.edge[8:158], MARGIN=1)
datesall1=lme(sralldates1~Habitat*Date, random=~1|Plot, data=commdata.edge)
anova(datesall1)
summary(datesall1)

commdata.edge$Habitat
colnames(commdata)
commdata$Habitat


colnames(commdata)=commdata[1,]
lme(Hdate1~Habitat+Plot, data=date1)
```


```{r RDA DATE1}
## Remove fruitflies from the analyses
#rdadate1=commdata[commdata$Date=="10.6.15",which(colnames(commdata)!="Drosophilidae..Drosophila"&colnames(commdata)!="Drosophilidae..Microdrosophila"&colnames(commdata)!="Drosophilidae..Mycodrosophila")]
rdadate1=date1[,c(1:80,84:233)]
dim(rdadate1)

#### All plots
tree.hel.soil=rdadate1
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
dim(rdadate1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:268])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:155]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:268])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:155]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32+tree.hel.all.pcnm$PCNM33+tree.hel.all.pcnm$PCNM34+tree.hel.all.pcnm$PCNM35+tree.hel.all.pcnm$PCNM36+tree.hel.all.pcnm$PCNM37+tree.hel.all.pcnm$PCNM38)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(234,232)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(rdadate1[,8:155]~rdadate1$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Core plots ####
tree.hel.soil=rdadate1[rdadate1$EP=="Core",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
dim(rdadate1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:249])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:155]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:249])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:155]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(249,232,231,236,234)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:155]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### CORE+EDGE ####
tree.hel.soil=rdadate1[rdadate1$EP=="Core"|rdadate1$EP=="Edge",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:262])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:155]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:262])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:155]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(258,232:234,238)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(rdadate1[,8:155]~rdadate1$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

## Variance partitioning
varpart(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(172,176)], tree.hel.all.pcnm[,c(234,254,246,233,241)])
## Check to see if it's significant when PCNM and soil come first.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(172,176)], tree.hel.all.pcnm[,c(234,254,246,233,241)])
anova(hel.rda.all.partpcnm)
# Yes.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(234,254,246,233,241)], tree.hel.all.pcnm[,c(172,176)])
anova(hel.rda.all.partsoil)
```
```{r RDA DATE2}
## Remove fruitflies from the analyses
#rdadate2=commdata[commdata$Date=="5.5.16",which(colnames(commdata)!="Drosophilidae..Drosophila"&colnames(commdata)!="Drosophilidae..Microdrosophila"&colnames(commdata)!="Drosophilidae..Mycodrosophila")]
rdadate2=date2[,c(1:80,84:233)]
dim(rdadate2)

#### All plots
tree.hel.soil=rdadate2
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:155]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:155]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,c(168,181,179)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


###INVERT BY TREE DATA
dim(rdadate1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:268])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(rdadate1[,8:155]~rdadate1$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Core plots ####
tree.hel.soil=rdadate2[rdadate2$EP=="Core",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:249])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:155]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### CORE+EDGE ####
tree.hel.soil=rdadate2[rdadate2$EP=="Core"|rdadate2$EP=="Edge",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:155]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:155]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,c(168,173,179)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:262])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(rdadate1[,8:155]~rdadate1$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```
```{r RDA DATE3}
## Remove fruitflies from the analyses
#rdadate3=commdata[commdata$Date=="8.10.16",which(colnames(commdata)!="Drosophilidae..Drosophila"&colnames(commdata)!="Drosophilidae..Microdrosophila"&colnames(commdata)!="Drosophilidae..Mycodrosophila")]
rdadate3=date3[,c(1:80,84:233)]
dim(rdadate3)

#### All plots
tree.hel.soil=rdadate3
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:155]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:155]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,c(165,178,180,175)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


###INVERT BY TREE DATA
dim(rdadate3)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:267])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(rdadate3[,8:155]~rdadate3$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Core plots ####
tree.hel.soil=rdadate3[rdadate3$EP=="Core",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
dim(rdadate3)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:249])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:155]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### CORE+EDGE ####
tree.hel.soil=rdadate3[rdadate3$EP=="Core"|rdadate3$EP=="Edge",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:261])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(rdadate3[,8:155]~rdadate3$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```
```{r RDA DATE4}
## Remove fruitflies from the analyses
#rdadate4=commdata[commdata$Date=="10.15.16",which(colnames(commdata)!="Drosophilidae..Drosophila"&colnames(commdata)!="Drosophilidae..Microdrosophila"&colnames(commdata)!="Drosophilidae..Mycodrosophila")]
rdadate4=date4[,c(1:80,84:233)]
dim(rdadate4)

#### All plots
tree.hel.soil=rdadate4
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:155]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:155]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,c(169,172,173,182,174)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


###INVERT BY TREE DATA
dim(rdadate4)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:155]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,195:229])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:155]~tree.hel.soil$ACERUB+tree.hel.soil$ACESAC+tree.hel.soil$ACESIL+tree.hel.soil$BETALL+tree.hel.soil$CARCAR+tree.hel.soil$CARCOR+tree.hel.soil$CAROVA+tree.hel.soil$CARTOM+tree.hel.soil$CORFLO+tree.hel.soil$CRAPUN+tree.hel.soil$FAGAME+tree.hel.soil$FRAAME+tree.hel.soil$FRANIG+tree.hel.soil$HAMVIR+tree.hel.soil$LIRTUP+tree.hel.soil$MAGACU+tree.hel.soil$MALCOR+tree.hel.soil$NYSSYL+tree.hel.soil$OSTVIR+tree.hel.soil$PLAOCC+tree.hel.soil$POPDEL+tree.hel.soil$POPGRA+tree.hel.soil$PRUSER+tree.hel.soil$QUEALB+tree.hel.soil$QUEBIC+tree.hel.soil$QUEPAL+tree.hel.soil$QUERUB+tree.hel.soil$SALNIG+tree.hel.soil$SASALB+tree.hel.soil$TILAME+tree.hel.soil$TOXVER+tree.hel.soil$ULMAME+tree.hel.soil$ULMRUB+tree.hel.soil$ULMSP+tree.hel.soil$VIBLEN)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
#hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,c(169,172,173,182,174)])
#anova(hel.rda.fulsoil)
#RsquareAdj(hel.rda.fulsoil)
#global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:263])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:155]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:263])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:155]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32+tree.hel.all.pcnm$PCNM33)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(232,257,252,243,237,255)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## Variance partitioning
varpart(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(232,257,252,243,237,255)],tree.hel.all.pcnm[,c(169,172,173,182,174)])
## Check to see if it's significant when PCNM and soil come first.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(169,172,173,182,174)], tree.hel.all.pcnm[,c(232,257,252,243,237,255)])
anova(hel.rda.all.partpcnm)
# Yes.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(232,257,252,243,237,255)], tree.hel.all.pcnm[,c(169,172,173,182,174)])
anova(hel.rda.all.partsoil)
# Yes.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(rdadate4[,8:155]~rdadate4$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Core plots ####
tree.hel.soil=rdadate4[rdadate4$EP=="Core",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
dim(rdadate4)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:249])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:155]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### CORE+EDGE ####
tree.hel.soil=rdadate4[rdadate4$EP=="Core"|rdadate4$EP=="Edge",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:155]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,165:189])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:155]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:155], tree.hel.soil[,c(169,172,173)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:155],tree.hel.soil[,c(195:229)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,193:194],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:259])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:155]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,231:259])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:155]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(251,233)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## Variance partitioning
varpart(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(251,233)],tree.hel.all.pcnm[,c(169,172,173)])
## Check to see if it's significant when PCNM and soil come first.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(169,172,173)], tree.hel.all.pcnm[,c(251,233)])
anova(hel.rda.all.partpcnm)
# Yes.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,8:155], tree.hel.all.pcnm[,c(251,233)], tree.hel.all.pcnm[,c(169,172,173)])
anova(hel.rda.all.partsoil)
# Yes.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(rdadate1[,8:155]~rdadate1$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```

Order-level decoupling (NEED TO DO: aranaea, coleoptera, diptera, hemiptera, hymenoptera, lepidoptera)
```{r Date 1}
#### Aranaea ####
aranaea.d1=date1[,c(1:4,8:20,159:233)]
#remove NAs.
aranaea.d1=na.omit(aranaea.d1)
#save a back-up for later, just in case.
aranaea.d1.2=aranaea.d1
##rda by soil
colnames(aranaea.d1)
hel.rda.fulsoil = rda(aranaea.d1[,5:17], aranaea.d1[,27:51])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d1[,5:17],aranaea.d1[,c(57:91)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d1)
all.geog.dist=vegdist(aranaea.d1[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:130])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d1[,5:17]~aranaea.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
aranaea.d1=aranaea.d1.2[aranaea.d1.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(aranaea.d1[,5:17], aranaea.d1[,27:51])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d1[,5:17],aranaea.d1[,57:91])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d1)
all.geog.dist=vegdist(aranaea.d1[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:124])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d1[,5:17]~aranaea.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
aranaea.d1=aranaea.d1.2[aranaea.d1.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(aranaea.d1[,5:17], aranaea.d1[,27:51])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(aranaea.d1[,5:17]~1)
hel.rda.fulsoil = rda(aranaea.d1[,5:17], aranaea.d1[,27:51])

hel.rda.fulsoil.full=rda(aranaea.d1[,5:17]~aranaea.d1$Ptot_long+aranaea.d1$Po_long+aranaea.d1$pH_long+aranaea.d1$moist_long+aranaea.d1$ps1_long+aranaea.d1$ps2_long+aranaea.d1$percc_long+aranaea.d1$cton_long+aranaea.d1$Ptot_nug+aranaea.d1$Po_nug+aranaea.d1$pH_nug+aranaea.d1$moist_nug+aranaea.d1$ps1_nug+aranaea.d1$ps2_nug+aranaea.d1$percc_nug+aranaea.d1$pomc_nug+aranaea.d1$cton_nug+aranaea.d1$Ptot_short+aranaea.d1$Po_short+aranaea.d1$pH_short+aranaea.d1$moist_short+aranaea.d1$ps1_short+aranaea.d1$percc_short+aranaea.d1$pomc_short+aranaea.d1$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(aranaea.d1[,5:17], aranaea.d1[,c(168,181,179)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d1[,5:17],aranaea.d1[,c(57:91)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d1)
all.geog.dist=vegdist(aranaea.d1[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:111])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d1[,5:17]~aranaea.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### Coleoptera ####
coleoptera.d1=date1[,c(1:4,21:67,159:233)]
#remove NAs.
coleoptera.d1=na.omit(coleoptera.d1)
#save a back-up for later, just in case.
coleoptera.d1.2=coleoptera.d1
##rda by soil
colnames(coleoptera.d1)
hel.rda.fulsoil = rda(coleoptera.d1[,5:51], coleoptera.d1[,61:85])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Just the variables forward selected
hel.rda.fulsoil = rda(aranaea.d1[,5:17], aranaea.d1[,c(168,181,179)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d1[,5:51],coleoptera.d1[,c(91:125)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d1)
all.geog.dist=vegdist(coleoptera.d1[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:164])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## Forward Selection
hel.rda.fulsoil.none = rda(tree.hel.all.pcnm[,5:51]~1)
.hel.rda.fulsoil = rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,119:134])

hel.rda.fulsoil.full=rda(tree.hel.all.pcnm[,5:51]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,c(128,134,130,139)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
#global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d1[,5:51]~coleoptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
coleoptera.d1=coleoptera.d1.2[coleoptera.d1.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(coleoptera.d1[,5:51], coleoptera.d1[,61:85])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d1[,5:51],coleoptera.d1[,91:125])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d1)
all.geog.dist=vegdist(coleoptera.d1[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:158])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,5:51]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:158])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,5:51]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,c(154,128,129,130,134)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d1[,5:51]~coleoptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
coleoptera.d1=coleoptera.d1.2[coleoptera.d1.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(coleoptera.d1[,5:51], coleoptera.d1[,61:85])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d1[,5:51],coleoptera.d1[,c(91:125)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d1)
all.geog.dist=vegdist(coleoptera.d1[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:145])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,5:51]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:145])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,5:51]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,c(145,127:130,132,135,143)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d1[,5:51]~coleoptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Diptera ####
diptera.d1=date1[,c(1:4,70:107,159:233)]
#remove NAs.
diptera.d1=na.omit(diptera.d1)
#save a back-up for later, just in case.
diptera.d1.2=diptera.d1
##rda by soil
colnames(diptera.d1)
hel.rda.fulsoil = rda(diptera.d1[,5:42], diptera.d1[,52:76])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d1[,5:42],diptera.d1[,c(82:116)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d1)
all.geog.dist=vegdist(diptera.d1[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:155])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d1[,5:42]~diptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
diptera.d1=diptera.d1.2[diptera.d1.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(diptera.d1[,5:42], diptera.d1[,52:76])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d1[,5:42],diptera.d1[,82:116])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d1)
all.geog.dist=vegdist(diptera.d1[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:149])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d1[,5:42]~diptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
diptera.d1=diptera.d1.2[diptera.d1.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(diptera.d1[,5:42], diptera.d1[,52:76])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d1[,5:42],diptera.d1[,c(82:116)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d1)
all.geog.dist=vegdist(diptera.d1[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:136])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d1[,5:42]~diptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Hemiptera ####
hemiptera.d1=date1[,c(1:4,109:117,159:233)]
#remove NAs.
hemiptera.d1=na.omit(hemiptera.d1)
#save a back-up for later, just in case.
hemiptera.d1.2=hemiptera.d1
##rda by soil
colnames(hemiptera.d1)
hel.rda.fulsoil = rda(hemiptera.d1[,5:13], hemiptera.d1[,23:47])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d1[,5:13],hemiptera.d1[,c(53:87)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d1)
all.geog.dist=vegdist(hemiptera.d1[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[,89:126])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d1[,5:13]~hemiptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
hemiptera.d1=hemiptera.d1.2[hemiptera.d1.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(hemiptera.d1[,5:13], hemiptera.d1[,23:47])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d1[,5:13],hemiptera.d1[,53:87])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d1)
all.geog.dist=vegdist(hemiptera.d1[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[,89:120])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d1[,5:13]~hemiptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
hemiptera.d1=hemiptera.d1.2[hemiptera.d1.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(hemiptera.d1[,5:13], hemiptera.d1[,23:47])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d1[,5:13],hemiptera.d1[,c(53:87)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d1)
all.geog.dist=vegdist(hemiptera.d1[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[89:107])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d1[,5:13]~hemiptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Hymenoptera ####
hymenoptera.d1=date1[,c(1:4,118:144,159:233)]
#remove NAs.
hymenoptera.d1=na.omit(hymenoptera.d1)
#save a back-up for later, just in case.
hymenoptera.d1.2=hymenoptera.d1
##rda by soil
colnames(hymenoptera.d1)
hel.rda.fulsoil = rda(hymenoptera.d1[,5:31], hymenoptera.d1[,41:65])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d1[,5:31],hymenoptera.d1[,c(71:105)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d1)
all.geog.dist=vegdist(hymenoptera.d1[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:144])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d1[,5:31]~hymenoptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
hymenoptera.d1=hymenoptera.d1.2[hymenoptera.d1.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(hymenoptera.d1[,5:31], hymenoptera.d1[,41:65])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d1[,5:31],hymenoptera.d1[,71:105])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d1)
all.geog.dist=vegdist(hymenoptera.d1[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:138])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d1[,5:31]~hymenoptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
hymenoptera.d1=hymenoptera.d1.2[hymenoptera.d1.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(hymenoptera.d1[,5:31], hymenoptera.d1[,41:65])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d1[,5:31],hymenoptera.d1[,c(71:105)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d1)
all.geog.dist=vegdist(hymenoptera.d1[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:125])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d1[,5:31]~hymenoptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Lepidoptera ####
lepidoptera.d1=date1[,c(1:4,145:149,159:233)]
#remove NAs.
lepidoptera.d1=na.omit(lepidoptera.d1)
#save a back-up for later, just in case.
lepidoptera.d1.2=lepidoptera.d1
##rda by soil
colnames(lepidoptera.d1)
hel.rda.fulsoil = rda(lepidoptera.d1[,5:9], lepidoptera.d1[,19:43])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d1[,5:9],lepidoptera.d1[,c(49:83)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d1)
all.geog.dist=vegdist(lepidoptera.d1[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:122])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d1[,5:9]~lepidoptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
lepidoptera.d1=lepidoptera.d1.2[lepidoptera.d1.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(lepidoptera.d1[,5:9], lepidoptera.d1[,19:43])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d1[,5:9],lepidoptera.d1[,49:83])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d1)
all.geog.dist=vegdist(lepidoptera.d1[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:116])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d1[,5:9]~lepidoptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
lepidoptera.d1=lepidoptera.d1.2[lepidoptera.d1.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(lepidoptera.d1[,5:9], lepidoptera.d1[,19:43])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d1[,5:9],lepidoptera.d1[,c(49:83)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d1)
all.geog.dist=vegdist(lepidoptera.d1[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d1,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d1,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:103])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d1[,5:9]~lepidoptera.d1$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```
```{r Date 2}
#### Aranaea ####
aranaea.d2=date2[,c(1:4,8:20,159:233)]
#remove NAs.
aranaea.d2=na.omit(aranaea.d2)
#save a back-up for later, just in case.
aranaea.d2.2=aranaea.d2
##rda by soil
colnames(aranaea.d2)
hel.rda.fulsoil = rda(aranaea.d2[,5:17], aranaea.d2[,27:51])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d2[,5:17],aranaea.d2[,c(57:91)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d2)
all.geog.dist=vegdist(aranaea.d2[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:130])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d2[,5:17]~aranaea.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
aranaea.d2=aranaea.d2.2[aranaea.d2.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(aranaea.d2[,5:17], aranaea.d2[,27:51])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d2[,5:17],aranaea.d2[,57:91])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d2)
all.geog.dist=vegdist(aranaea.d2[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:124])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d2[,5:17]~aranaea.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
aranaea.d2=aranaea.d2.2[aranaea.d2.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(aranaea.d2[,5:17], aranaea.d2[,27:51])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d2[,5:17],aranaea.d2[,c(57:91)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d2)
all.geog.dist=vegdist(aranaea.d2[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:111])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d2[,5:17]~aranaea.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### Coleoptera ####
coleoptera.d2=date2[,c(1:4,21:67,159:233)]
#remove NAs.
coleoptera.d2=na.omit(coleoptera.d2)
#save a back-up for later, just in case.
coleoptera.d2.2=coleoptera.d2
##rda by soil
colnames(coleoptera.d2)
hel.rda.fulsoil = rda(coleoptera.d2[,5:51], coleoptera.d2[,61:85])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(coleoptera.d2[,5:51]~1)
hel.rda.fulsoil = rda(coleoptera.d2[,5:51], coleoptera.d2[,61:85])

hel.rda.fulsoil.full=rda(coleoptera.d2[,5:51]~coleoptera.d2$Ptot_long+coleoptera.d2$Po_long+coleoptera.d2$pH_long+coleoptera.d2$moist_long+coleoptera.d2$ps1_long+coleoptera.d2$ps2_long+coleoptera.d2$percc_long+coleoptera.d2$cton_long+coleoptera.d2$Ptot_nug+coleoptera.d2$Po_nug+coleoptera.d2$pH_nug+coleoptera.d2$moist_nug+coleoptera.d2$ps1_nug+coleoptera.d2$ps2_nug+coleoptera.d2$percc_nug+coleoptera.d2$pomc_nug+coleoptera.d2$cton_nug+coleoptera.d2$Ptot_short+coleoptera.d2$Po_short+coleoptera.d2$pH_short+coleoptera.d2$moist_short+coleoptera.d2$ps1_short+coleoptera.d2$percc_short+coleoptera.d2$pomc_short+coleoptera.d2$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(coleoptera.d2[,5:51], coleoptera.d2[,c(77,64,75)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d2[,5:51],coleoptera.d2[,c(91:125)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d2)
all.geog.dist=vegdist(coleoptera.d2[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:164])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d2[,5:51]~coleoptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
coleoptera.d2=coleoptera.d2.2[coleoptera.d2.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(coleoptera.d2[,5:51], coleoptera.d2[,61:85])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d2[,5:51],coleoptera.d2[,91:125])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d2)
all.geog.dist=vegdist(coleoptera.d2[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:158])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d2[,5:51]~coleoptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
coleoptera.d2=coleoptera.d2.2[coleoptera.d2.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(coleoptera.d2[,5:51], coleoptera.d2[,61:85])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d2[,5:51],coleoptera.d2[,c(91:125)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d2)
all.geog.dist=vegdist(coleoptera.d2[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:145])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d2[,5:51]~coleoptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Diptera ####
diptera.d2=date2[,c(1:4,70:107,159:233)]
#remove NAs.
diptera.d2=na.omit(diptera.d2)
#save a back-up for later, just in case.
diptera.d2.2=diptera.d2
##rda by soil
colnames(diptera.d2)
hel.rda.fulsoil = rda(diptera.d2[,5:42], diptera.d2[,52:76])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d2[,5:42],diptera.d2[,c(82:116)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d2)
all.geog.dist=vegdist(diptera.d2[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:155])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d2[,5:42]~diptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
diptera.d2=diptera.d2.2[diptera.d2.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(diptera.d2[,5:42], diptera.d2[,52:76])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d2[,5:42],diptera.d2[,82:116])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d2)
all.geog.dist=vegdist(diptera.d2[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:149])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d2[,5:42]~diptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
diptera.d2=diptera.d2.2[diptera.d2.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(diptera.d2[,5:42], diptera.d2[,52:76])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d2[,5:42],diptera.d2[,c(82:116)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d2)
all.geog.dist=vegdist(diptera.d2[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:136])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d2[,5:42]~diptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### Hemiptera ####
hemiptera.d2=date2[,c(1:4,109:117,159:233)]
#remove NAs.
hemiptera.d2=na.omit(hemiptera.d2)
#save a back-up for later, just in case.
hemiptera.d2.2=hemiptera.d2
##rda by soil
colnames(hemiptera.d2)
hel.rda.fulsoil = rda(hemiptera.d2[,5:13], hemiptera.d2[,23:47])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d2[,5:13],hemiptera.d2[,c(53:87)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d2)
all.geog.dist=vegdist(hemiptera.d2[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[,89:126])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d2[,5:13]~hemiptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
hemiptera.d2=hemiptera.d2.2[hemiptera.d2.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(hemiptera.d2[,5:13], hemiptera.d2[,23:47])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d2[,5:13],hemiptera.d2[,53:87])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d2)
all.geog.dist=vegdist(hemiptera.d2[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[,89:120])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d2[,5:13]~hemiptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
hemiptera.d2=hemiptera.d2.2[hemiptera.d2.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(hemiptera.d2[,5:13], hemiptera.d2[,23:47])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d2[,5:13],hemiptera.d2[,c(53:87)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d2)
all.geog.dist=vegdist(hemiptera.d2[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[89:107])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d2[,5:13]~hemiptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Hymenoptera ####
hymenoptera.d2=date2[,c(1:4,118:144,159:233)]
#remove NAs.
hymenoptera.d2=na.omit(hymenoptera.d2)
#save a back-up for later, just in case.
hymenoptera.d2.2=hymenoptera.d2
##rda by soil
colnames(hymenoptera.d2)
hel.rda.fulsoil = rda(hymenoptera.d2[,5:31], hymenoptera.d2[,41:65])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d2[,5:31],hymenoptera.d2[,c(71:105)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d2)
all.geog.dist=vegdist(hymenoptera.d2[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:144])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d2[,5:31]~hymenoptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
hymenoptera.d2=hymenoptera.d2.2[hymenoptera.d2.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(hymenoptera.d2[,5:31], hymenoptera.d2[,41:65])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d2[,5:31],hymenoptera.d2[,71:105])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d2)
all.geog.dist=vegdist(hymenoptera.d2[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:138])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d2[,5:31]~hymenoptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
hymenoptera.d2=hymenoptera.d2.2[hymenoptera.d2.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(hymenoptera.d2[,5:31], hymenoptera.d2[,41:65])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d2[,5:31],hymenoptera.d2[,c(71:105)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d2)
all.geog.dist=vegdist(hymenoptera.d2[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:125])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d2[,5:31]~hymenoptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Lepidoptera ####
lepidoptera.d2=date2[,c(1:4,145:149,159:233)]
#remove NAs.
lepidoptera.d2=na.omit(lepidoptera.d2)
#save a back-up for later, just in case.
lepidoptera.d2.2=lepidoptera.d2
##rda by soil
colnames(lepidoptera.d2)
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9], lepidoptera.d2[,19:43])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(lepidoptera.d2[,5:9]~1)
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9], lepidoptera.d2[,27:51])

hel.rda.fulsoil.full=rda(lepidoptera.d2[,5:9]~lepidoptera.d2$Ptot_long+lepidoptera.d2$Po_long+lepidoptera.d2$pH_long+lepidoptera.d2$moist_long+lepidoptera.d2$ps1_long+lepidoptera.d2$ps2_long+lepidoptera.d2$percc_long+lepidoptera.d2$cton_long+lepidoptera.d2$Ptot_nug+lepidoptera.d2$Po_nug+lepidoptera.d2$pH_nug+lepidoptera.d2$moist_nug+lepidoptera.d2$ps1_nug+lepidoptera.d2$ps2_nug+lepidoptera.d2$percc_nug+lepidoptera.d2$pomc_nug+lepidoptera.d2$cton_nug+lepidoptera.d2$Ptot_short+lepidoptera.d2$Po_short+lepidoptera.d2$pH_short+lepidoptera.d2$moist_short+lepidoptera.d2$ps1_short+lepidoptera.d2$percc_short+lepidoptera.d2$pomc_short+lepidoptera.d2$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9], lepidoptera.d2[,c(33,27)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9],lepidoptera.d2[,c(49:83)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d2)
all.geog.dist=vegdist(lepidoptera.d2[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:122])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9]~lepidoptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
lepidoptera.d2=lepidoptera.d2.2[lepidoptera.d2.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9], lepidoptera.d2[,19:43])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(lepidoptera.d2[,5:9]~1)
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9], lepidoptera.d2[,27:51])

hel.rda.fulsoil.full=rda(lepidoptera.d2[,5:9]~lepidoptera.d2$Ptot_long+lepidoptera.d2$Po_long+lepidoptera.d2$pH_long+lepidoptera.d2$moist_long+lepidoptera.d2$ps1_long+lepidoptera.d2$ps2_long+lepidoptera.d2$percc_long+lepidoptera.d2$cton_long+lepidoptera.d2$Ptot_nug+lepidoptera.d2$Po_nug+lepidoptera.d2$pH_nug+lepidoptera.d2$moist_nug+lepidoptera.d2$ps1_nug+lepidoptera.d2$ps2_nug+lepidoptera.d2$percc_nug+lepidoptera.d2$pomc_nug+lepidoptera.d2$cton_nug+lepidoptera.d2$Ptot_short+lepidoptera.d2$Po_short+lepidoptera.d2$pH_short+lepidoptera.d2$moist_short+lepidoptera.d2$ps1_short+lepidoptera.d2$percc_short+lepidoptera.d2$pomc_short+lepidoptera.d2$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9], lepidoptera.d2[,c(19,39,33)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9],lepidoptera.d2[,49:83])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d2)
all.geog.dist=vegdist(lepidoptera.d2[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:116])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9]~lepidoptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
lepidoptera.d2=lepidoptera.d2.2[lepidoptera.d2.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9], lepidoptera.d2[,19:43])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9],lepidoptera.d2[,c(49:83)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(lepidoptera.d2[,5:9]~1)
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9], lepidoptera.d2[,49:83])

hel.rda.fulsoil.full=rda(lepidoptera.d2[,5:9]~lepidoptera.d2$ACERUB+lepidoptera.d2$ACESAC+lepidoptera.d2$ACESIL+lepidoptera.d2$BETALL+lepidoptera.d2$CARCAR+lepidoptera.d2$CARCOR+lepidoptera.d2$CAROVA+lepidoptera.d2$CARTOM+lepidoptera.d2$CORFLO+lepidoptera.d2$CRAPUN+lepidoptera.d2$FAGAME+lepidoptera.d2$FRAAME+lepidoptera.d2$FRANIG+lepidoptera.d2$HAMVIR+lepidoptera.d2$LIRTUP+lepidoptera.d2$MAGACU+lepidoptera.d2$MALCOR+lepidoptera.d2$NYSSYL+lepidoptera.d2$OSTVIR+lepidoptera.d2$PLAOCC+lepidoptera.d2$POPDEL+lepidoptera.d2$POPGRA+lepidoptera.d2$PRUSER+lepidoptera.d2$QUEALB+lepidoptera.d2$QUEBIC+lepidoptera.d2$QUEPAL+lepidoptera.d2$QUERUB+lepidoptera.d2$SALNIG+lepidoptera.d2$SASALB+lepidoptera.d2$TILAME+lepidoptera.d2$TOXVER+lepidoptera.d2$ULMAME+lepidoptera.d2$ULMRUB+lepidoptera.d2$ULMSP+lepidoptera.d2$VIBLEN)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
#hel.rda.fulsoil = rda(lepidoptera.d2[,5:9], lepidoptera.d2[,c(169,172,173,182,174)])
#anova(hel.rda.fulsoil)
#RsquareAdj(hel.rda.fulsoil)
#global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d2)
all.geog.dist=vegdist(lepidoptera.d2[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d2,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d2,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:103])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d2[,5:9]~lepidoptera.d2$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```
```{r Date 3}
#### Aranaea ####
aranaea.d3=date3[,c(1:4,8:20,159:233)]
#remove NAs.
aranaea.d3=na.omit(aranaea.d3)
#save a back-up for later, just in case.
aranaea.d3.2=aranaea.d3
##rda by soil
colnames(aranaea.d3)
hel.rda.fulsoil = rda(aranaea.d3[,5:17], aranaea.d3[,27:51])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d3[,5:17],aranaea.d3[,c(57:91)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d3)
all.geog.dist=vegdist(aranaea.d3[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:129])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d3[,5:17]~aranaea.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
aranaea.d3=aranaea.d3.2[aranaea.d3.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(aranaea.d3[,5:17], aranaea.d3[,27:51])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d3[,5:17],aranaea.d3[,57:91])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d3)
all.geog.dist=vegdist(aranaea.d3[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:123])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d3[,5:17]~aranaea.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE PLOTS####
aranaea.d3=aranaea.d3.2[aranaea.d3.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(aranaea.d3[,5:17], aranaea.d3[,27:51])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(aranaea.d3[,5:17]~1)
hel.rda.fulsoil = rda(aranaea.d3[,5:17], aranaea.d3[,27:51])

hel.rda.fulsoil.full=rda(aranaea.d3[,5:17]~aranaea.d3$Ptot_long+aranaea.d3$Po_long+aranaea.d3$pH_long+aranaea.d3$moist_long+aranaea.d3$ps1_long+aranaea.d3$ps2_long+aranaea.d3$percc_long+aranaea.d3$cton_long+aranaea.d3$Ptot_nug+aranaea.d3$Po_nug+aranaea.d3$pH_nug+aranaea.d3$moist_nug+aranaea.d3$ps1_nug+aranaea.d3$ps2_nug+aranaea.d3$percc_nug+aranaea.d3$pomc_nug+aranaea.d3$cton_nug+aranaea.d3$Ptot_short+aranaea.d3$Po_short+aranaea.d3$pH_short+aranaea.d3$moist_short+aranaea.d3$ps1_short+aranaea.d3$percc_short+aranaea.d3$pomc_short+aranaea.d3$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(aranaea.d3[,5:17], aranaea.d3[,c(30)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d3[,5:17],aranaea.d3[,c(57:91)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(aranaea.d3[,5:17]~1)
hel.rda.fulsoil = rda(aranaea.d3[,5:17], aranaea.d3[,57:91])

hel.rda.fulsoil.full=rda(aranaea.d3[,5:17]~aranaea.d3$ACERUB+aranaea.d3$ACESAC+aranaea.d3$ACESIL+aranaea.d3$BETALL+aranaea.d3$CARCAR+aranaea.d3$CARCOR+aranaea.d3$CAROVA+aranaea.d3$CARTOM+aranaea.d3$CORFLO+aranaea.d3$CRAPUN+aranaea.d3$FAGAME+aranaea.d3$FRAAME+aranaea.d3$FRANIG+aranaea.d3$HAMVIR+aranaea.d3$LIRTUP+aranaea.d3$MAGACU+aranaea.d3$MALCOR+aranaea.d3$NYSSYL+aranaea.d3$OSTVIR+aranaea.d3$PLAOCC+aranaea.d3$POPDEL+aranaea.d3$POPGRA+aranaea.d3$PRUSER+aranaea.d3$QUEALB+aranaea.d3$QUEBIC+aranaea.d3$QUEPAL+aranaea.d3$QUERUB+aranaea.d3$SALNIG+aranaea.d3$SASALB+aranaea.d3$TILAME+aranaea.d3$TOXVER+aranaea.d3$ULMAME+aranaea.d3$ULMRUB+aranaea.d3$ULMSP+aranaea.d3$VIBLEN)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
#hel.rda.fulsoil = rda(aranaea.d3[,5:17], aranaea.d3[,c(169,172,173,182,174)])
#anova(hel.rda.fulsoil)
#RsquareAdj(hel.rda.fulsoil)
#global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d3)
all.geog.dist=vegdist(aranaea.d3[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:111])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

## Variance partitioning
varpart(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(30)], tree.hel.all.pcnm[,c(57:91)])
## Check to see if it's significant when PCNM and soil come first.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(91:93,97:98)], tree.hel.all.pcnm[,c(23,43,31)])
anova(hel.rda.all.partpcnm)
# Yes.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(23,43,31)], tree.hel.all.pcnm[,c(91:93,97:98)])
anova(hel.rda.all.partsoil)
# Yes.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d3[,5:17]~aranaea.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### Coleoptera ####
coleoptera.d3=date3[,c(1:4,21:67,159:233)]
#remove NAs.
coleoptera.d3=na.omit(coleoptera.d3)
#save a back-up for later, just in case.
coleoptera.d3.2=coleoptera.d3
##rda by soil
colnames(coleoptera.d3)
hel.rda.fulsoil = rda(coleoptera.d3[,5:51], coleoptera.d3[,61:85])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d3[,5:51],coleoptera.d3[,c(91:125)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d3)
all.geog.dist=vegdist(coleoptera.d3[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:163])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d3[,5:51]~coleoptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
coleoptera.d3=coleoptera.d3.2[coleoptera.d3.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(coleoptera.d3[,5:51], coleoptera.d3[,61:85])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d3[,5:51],coleoptera.d3[,91:125])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d3)
all.geog.dist=vegdist(coleoptera.d3[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:157])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d3[,5:51]~coleoptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
coleoptera.d3=coleoptera.d3.2[coleoptera.d3.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(coleoptera.d3[,5:51], coleoptera.d3[,61:85])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d3[,5:51],coleoptera.d3[,c(91:125)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d3)
all.geog.dist=vegdist(coleoptera.d3[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:145])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d3[,5:51]~coleoptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Diptera ####
diptera.d3=date3[,c(1:4,70:107,159:233)]
#remove NAs.
diptera.d3=na.omit(diptera.d3)
#save a back-up for later, just in case.
diptera.d3.2=diptera.d3
##rda by soil
colnames(diptera.d3)
hel.rda.fulsoil = rda(diptera.d3[,5:42], diptera.d3[,52:76])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d3[,5:42],diptera.d3[,c(82:116)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d3)
all.geog.dist=vegdist(diptera.d3[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:154])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,5:42]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:154])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,5:42]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32+tree.hel.all.pcnm$PCNM33+tree.hel.all.pcnm$PCNM34+tree.hel.all.pcnm$PCNM35+tree.hel.all.pcnm$PCNM36+tree.hel.all.pcnm$PCNM37)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,c(128,143:146)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d3[,5:42]~diptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
diptera.d3=diptera.d3.2[diptera.d3.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(diptera.d3[,5:42], diptera.d3[,52:76])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d3[,5:42],diptera.d3[,82:116])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d3)
all.geog.dist=vegdist(diptera.d3[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:148])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d3[,5:42]~diptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
diptera.d3=diptera.d3.2[diptera.d3.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(diptera.d3[,5:42], diptera.d3[,52:76])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(diptera.d3[,5:42]~1)
hel.rda.fulsoil = rda(diptera.d3[,5:42], diptera.d3[,52:76])

hel.rda.fulsoil.full=rda(diptera.d3[,5:42]~diptera.d3$Ptot_long+diptera.d3$Po_long+diptera.d3$pH_long+diptera.d3$moist_long+diptera.d3$ps1_long+diptera.d3$ps2_long+diptera.d3$percc_long+diptera.d3$cton_long+diptera.d3$Ptot_nug+diptera.d3$Po_nug+diptera.d3$pH_nug+diptera.d3$moist_nug+diptera.d3$ps1_nug+diptera.d3$ps2_nug+diptera.d3$percc_nug+diptera.d3$pomc_nug+diptera.d3$cton_nug+diptera.d3$Ptot_short+diptera.d3$Po_short+diptera.d3$pH_short+diptera.d3$moist_short+diptera.d3$ps1_short+diptera.d3$percc_short+diptera.d3$pomc_short+diptera.d3$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(diptera.d3[,5:42], diptera.d3[,c(62,68)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d3[,5:42],diptera.d3[,c(82:116)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d3)
all.geog.dist=vegdist(diptera.d3[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:136])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d3[,5:42]~diptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### Hemiptera ####
hemiptera.d3=date3[,c(1:4,109:117,159:233)]
#remove NAs.
hemiptera.d3=na.omit(hemiptera.d3)
#save a back-up for later, just in case.
hemiptera.d3.2=hemiptera.d3
##rda by soil
colnames(hemiptera.d3)
hel.rda.fulsoil = rda(hemiptera.d3[,5:13], hemiptera.d3[,23:47])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(hemiptera.d3[,5:13]~1)
hel.rda.fulsoil = rda(hemiptera.d3[,5:13], hemiptera.d3[,23:47])

hel.rda.fulsoil.full=rda(hemiptera.d3[,5:13]~hemiptera.d3$Ptot_long+hemiptera.d3$Po_long+hemiptera.d3$pH_long+hemiptera.d3$moist_long+hemiptera.d3$ps1_long+hemiptera.d3$ps2_long+hemiptera.d3$percc_long+hemiptera.d3$cton_long+hemiptera.d3$Ptot_nug+hemiptera.d3$Po_nug+hemiptera.d3$pH_nug+hemiptera.d3$moist_nug+hemiptera.d3$ps1_nug+hemiptera.d3$ps2_nug+hemiptera.d3$percc_nug+hemiptera.d3$pomc_nug+hemiptera.d3$cton_nug+hemiptera.d3$Ptot_short+hemiptera.d3$Po_short+hemiptera.d3$pH_short+hemiptera.d3$moist_short+hemiptera.d3$ps1_short+hemiptera.d3$percc_short+hemiptera.d3$pomc_short+hemiptera.d3$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(hemiptera.d3[,5:13], hemiptera.d3[,c(26,31,39)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d3[,5:13],hemiptera.d3[,c(53:87)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d3)
all.geog.dist=vegdist(hemiptera.d3[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[,89:125])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d3[,5:13]~hemiptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
hemiptera.d3=hemiptera.d3.2[hemiptera.d3.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(hemiptera.d3[,5:13], hemiptera.d3[,23:47])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d3[,5:13],hemiptera.d3[,53:87])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d3)
all.geog.dist=vegdist(hemiptera.d3[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[,89:119])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,5:13]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[,89:119])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,5:13]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[,c(113:114,90)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d3[,5:13]~hemiptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
hemiptera.d3=hemiptera.d3.2[hemiptera.d3.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(hemiptera.d3[,5:13], hemiptera.d3[,23:47])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d3[,5:13],hemiptera.d3[,c(53:87)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d3)
all.geog.dist=vegdist(hemiptera.d3[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[89:107])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d3[,5:13]~hemiptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Hymenoptera ####
hymenoptera.d3=date3[,c(1:4,118:144,159:233)]
#remove NAs.
hymenoptera.d3=na.omit(hymenoptera.d3)
#save a back-up for later, just in case.
hymenoptera.d3.2=hymenoptera.d3
##rda by soil
colnames(hymenoptera.d3)
hel.rda.fulsoil = rda(hymenoptera.d3[,5:31], hymenoptera.d3[,41:65])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d3[,5:31],hymenoptera.d3[,c(71:105)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(hymenoptera.d3[,5:31]~1)
hel.rda.fulsoil = rda(hymenoptera.d3[,5:31], hymenoptera.d3[,71:105])

hel.rda.fulsoil.full=rda(hymenoptera.d3[,5:31]~hymenoptera.d3$ACERUB+hymenoptera.d3$ACESAC+hymenoptera.d3$ACESIL+hymenoptera.d3$BETALL+hymenoptera.d3$CARCAR+hymenoptera.d3$CARCOR+hymenoptera.d3$CAROVA+hymenoptera.d3$CARTOM+hymenoptera.d3$CORFLO+hymenoptera.d3$CRAPUN+hymenoptera.d3$FAGAME+hymenoptera.d3$FRAAME+hymenoptera.d3$FRANIG+hymenoptera.d3$HAMVIR+hymenoptera.d3$LIRTUP+hymenoptera.d3$MAGACU+hymenoptera.d3$MALCOR+hymenoptera.d3$NYSSYL+hymenoptera.d3$OSTVIR+hymenoptera.d3$PLAOCC+hymenoptera.d3$POPDEL+hymenoptera.d3$POPGRA+hymenoptera.d3$PRUSER+hymenoptera.d3$QUEALB+hymenoptera.d3$QUEBIC+hymenoptera.d3$QUEPAL+hymenoptera.d3$QUERUB+hymenoptera.d3$SALNIG+hymenoptera.d3$SASALB+hymenoptera.d3$TILAME+hymenoptera.d3$TOXVER+hymenoptera.d3$ULMAME+hymenoptera.d3$ULMRUB+hymenoptera.d3$ULMSP+hymenoptera.d3$VIBLEN)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
#hel.rda.fulsoil = rda(hymenoptera.d3[,5:31], hymenoptera.d3[,c(169,172,173,182,174)])
#anova(hel.rda.fulsoil)
#RsquareAdj(hel.rda.fulsoil)
#global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d3)
all.geog.dist=vegdist(hymenoptera.d3[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:143])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d3[,5:31]~hymenoptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
hymenoptera.d3=hymenoptera.d3.2[hymenoptera.d3.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(hymenoptera.d3[,5:31], hymenoptera.d3[,41:65])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d3[,5:31],hymenoptera.d3[,71:105])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d3)
all.geog.dist=vegdist(hymenoptera.d3[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:137])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d3[,5:31]~hymenoptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
hymenoptera.d3=hymenoptera.d3.2[hymenoptera.d3.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(hymenoptera.d3[,5:31], hymenoptera.d3[,41:65])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d3[,5:31],hymenoptera.d3[,c(71:105)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d3)
all.geog.dist=vegdist(hymenoptera.d3[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:125])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d3[,5:31]~hymenoptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Lepidoptera ####
lepidoptera.d3=date3[,c(1:4,145:149,159:233)]
#remove NAs.
lepidoptera.d3=na.omit(lepidoptera.d3)
#save a back-up for later, just in case.
lepidoptera.d3.2=lepidoptera.d3
##rda by soil
colnames(lepidoptera.d3)
hel.rda.fulsoil = rda(lepidoptera.d3[,5:9], lepidoptera.d3[,19:43])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d3[,5:9],lepidoptera.d3[,c(49:83)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d3)
all.geog.dist=vegdist(lepidoptera.d3[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:121])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d3[,5:9]~lepidoptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
lepidoptera.d3=lepidoptera.d3.2[lepidoptera.d3.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(lepidoptera.d3[,5:9], lepidoptera.d3[,19:43])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d3[,5:9],lepidoptera.d3[,49:83])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d3)
all.geog.dist=vegdist(lepidoptera.d3[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:115])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d3[,5:9]~lepidoptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
lepidoptera.d3=lepidoptera.d3.2[lepidoptera.d3.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(lepidoptera.d3[,5:9], lepidoptera.d3[,19:43])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d3[,5:9],lepidoptera.d3[,c(49:83)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d3)
all.geog.dist=vegdist(lepidoptera.d3[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d3,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d3,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:103])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d3[,5:9]~lepidoptera.d3$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```
```{r Date 4}
#### Aranaea ####
aranaea.d4=date4[,c(1:4,8:20,159:233)]
#remove NAs.
aranaea.d4=na.omit(aranaea.d4)
#save a back-up for later, just in case.
aranaea.d4.2=aranaea.d4
##rda by soil
colnames(aranaea.d4)
hel.rda.fulsoil = rda(aranaea.d4[,5:17], aranaea.d4[,27:51])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d4[,5:17],aranaea.d4[,c(57:91)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d4)
all.geog.dist=vegdist(aranaea.d4[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:125])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d4[,5:17]~aranaea.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
aranaea.d4=aranaea.d4.2[aranaea.d4.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(aranaea.d4[,5:17], aranaea.d4[,27:51])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d4[,5:17],aranaea.d4[,57:91])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d4)
all.geog.dist=vegdist(aranaea.d4[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:122])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d4[,5:17]~aranaea.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
aranaea.d4=aranaea.d4.2[aranaea.d4.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(aranaea.d4[,5:17], aranaea.d4[,27:51])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(aranaea.d4[,5:17],aranaea.d4[,c(57:91)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(aranaea.d4)
all.geog.dist=vegdist(aranaea.d4[,55:56],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(aranaea.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(aranaea.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:17], tree.hel.all.pcnm[,93:111])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(aranaea.d4[,5:17]~aranaea.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### Coleoptera ####
coleoptera.d4=date4[,c(1:4,21:67,159:233)]
#remove NAs.
coleoptera.d4=na.omit(coleoptera.d4)
#save a back-up for later, just in case.
coleoptera.d4.2=coleoptera.d4
##rda by soil
colnames(coleoptera.d4)
hel.rda.fulsoil = rda(coleoptera.d4[,5:51], coleoptera.d4[,61:85])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d4[,5:51],coleoptera.d4[,c(91:125)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d4)
all.geog.dist=vegdist(coleoptera.d4[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:159])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d4[,5:51]~coleoptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
coleoptera.d4=coleoptera.d4.2[coleoptera.d4.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(coleoptera.d4[,5:51], coleoptera.d4[,61:85])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d4[,5:51],coleoptera.d4[,91:125])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d4)
all.geog.dist=vegdist(coleoptera.d4[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:156])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d4[,5:51]~coleoptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
coleoptera.d4=coleoptera.d4.2[coleoptera.d4.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(coleoptera.d4[,5:51], coleoptera.d4[,61:85])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(coleoptera.d4[,5:51],coleoptera.d4[,c(91:125)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(coleoptera.d4)
all.geog.dist=vegdist(coleoptera.d4[,89:90],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(coleoptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(coleoptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:51], tree.hel.all.pcnm[,127:145])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(coleoptera.d4[,5:51]~coleoptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Diptera ####
diptera.d4=date4[,c(1:4,70:107,159:233)]
#remove NAs.
diptera.d4=na.omit(diptera.d4)
#save a back-up for later, just in case.
diptera.d4.2=diptera.d4
##rda by soil
colnames(diptera.d4)
hel.rda.fulsoil = rda(diptera.d4[,5:42], diptera.d4[,52:76])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d4[,5:42],diptera.d4[,c(82:116)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d4)
all.geog.dist=vegdist(diptera.d4[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:150])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d4[,5:42]~diptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
diptera.d4=diptera.d4.2[diptera.d4.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(diptera.d4[,5:42], diptera.d4[,52:76])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d4[,5:42],diptera.d4[,82:116])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d4)
all.geog.dist=vegdist(diptera.d4[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:147])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d4[,5:42]~diptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
diptera.d4=diptera.d4.2[diptera.d4.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(diptera.d4[,5:42], diptera.d4[,52:76])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(diptera.d4[,5:42],diptera.d4[,c(82:116)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(diptera.d4)
all.geog.dist=vegdist(diptera.d4[,80:81],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(diptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(diptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:42], tree.hel.all.pcnm[,118:136])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(diptera.d4[,5:42]~diptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


#### Hemiptera ####
hemiptera.d4=date4[,c(1:4,109:117,159:233)]
#remove NAs.
hemiptera.d4=na.omit(hemiptera.d4)
#save a back-up for later, just in case.
hemiptera.d4.2=hemiptera.d4
##rda by soil
colnames(hemiptera.d4)
hel.rda.fulsoil = rda(hemiptera.d4[,5:13], hemiptera.d4[,23:47])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d4[,5:13],hemiptera.d4[,c(53:87)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d4)
all.geog.dist=vegdist(hemiptera.d4[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[,89:121])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d4[,5:13]~hemiptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
hemiptera.d4=hemiptera.d4.2[hemiptera.d4.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(hemiptera.d4[,5:13], hemiptera.d4[,23:47])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d4[,5:13],hemiptera.d4[,53:87])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d4)
all.geog.dist=vegdist(hemiptera.d4[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[,89:118])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d4[,5:13]~hemiptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
hemiptera.d4=hemiptera.d4.2[hemiptera.d4.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(hemiptera.d4[,5:13], hemiptera.d4[,23:47])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hemiptera.d4[,5:13],hemiptera.d4[,c(53:87)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hemiptera.d4)
all.geog.dist=vegdist(hemiptera.d4[,51:52],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hemiptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hemiptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:13], tree.hel.all.pcnm[89:107])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hemiptera.d4[,5:13]~hemiptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Hymenoptera ####
hymenoptera.d4=date4[,c(1:4,118:144,159:233)]
#remove NAs.
hymenoptera.d4=na.omit(hymenoptera.d4)
#save a back-up for later, just in case.
hymenoptera.d4.2=hymenoptera.d4
##rda by soil
colnames(hymenoptera.d4)
hel.rda.fulsoil = rda(hymenoptera.d4[,5:31], hymenoptera.d4[,41:65])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d4[,5:31],hymenoptera.d4[,c(71:105)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d4)
all.geog.dist=vegdist(hymenoptera.d4[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:139])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d4[,5:31]~hymenoptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
hymenoptera.d4=hymenoptera.d4.2[hymenoptera.d4.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(hymenoptera.d4[,5:31], hymenoptera.d4[,41:65])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d4[,5:31],hymenoptera.d4[,71:105])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d4)
all.geog.dist=vegdist(hymenoptera.d4[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:136])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d4[,5:31]~hymenoptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
hymenoptera.d4=hymenoptera.d4.2[hymenoptera.d4.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(hymenoptera.d4[,5:31], hymenoptera.d4[,41:65])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(hymenoptera.d4[,5:31],hymenoptera.d4[,c(71:105)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(hymenoptera.d4)
all.geog.dist=vegdist(hymenoptera.d4[,69:70],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(hymenoptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(hymenoptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:31], tree.hel.all.pcnm[,107:125])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## no variance partitioning needed because no significance/soil effect.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(hymenoptera.d4[,5:31]~hymenoptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Lepidoptera ####
lepidoptera.d4=date4[,c(1:4,145:149,159:233)]
#remove NAs.
lepidoptera.d4=na.omit(lepidoptera.d4)
#save a back-up for later, just in case.
lepidoptera.d4.2=lepidoptera.d4
##rda by soil
colnames(lepidoptera.d4)
hel.rda.fulsoil = rda(lepidoptera.d4[,5:9], lepidoptera.d4[,19:43])
anova(hel.rda.fulsoil)#AYO
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d4[,5:9],lepidoptera.d4[,c(49:83)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d4)
all.geog.dist=vegdist(lepidoptera.d4[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:117])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d4[,5:9]~lepidoptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

####CORE+EDGE####
lepidoptera.d4=lepidoptera.d4.2[lepidoptera.d4.2$EP!="Trans",]
##rda by soil
hel.rda.fulsoil = rda(lepidoptera.d4[,5:9], lepidoptera.d4[,19:43])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d4[,5:9],lepidoptera.d4[,49:83])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d4)
all.geog.dist=vegdist(lepidoptera.d4[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:114])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
## Variance partitioning - Not needed because there's no pcnm effect

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d4[,5:9]~lepidoptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


####CORE PLOTS####
lepidoptera.d4=lepidoptera.d4.2[lepidoptera.d4.2$EP=="Core",]
##rda by soil
hel.rda.fulsoil = rda(lepidoptera.d4[,5:9], lepidoptera.d4[,19:43])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by trees
hel.rda.fulsoil = rda(lepidoptera.d4[,5:9],lepidoptera.d4[,c(49:83)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Pitfall inverts by PCNM
colnames(lepidoptera.d4)
all.geog.dist=vegdist(lepidoptera.d4[,47:48],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(lepidoptera.d4,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(lepidoptera.d4,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,5:9], tree.hel.all.pcnm[,85:103])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(lepidoptera.d4[,5:9]~lepidoptera.d4$TerrType)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```

```{r indval}
sppabun=commdata[,1:158]
cores=sppabun[sppabun$EP=="Core",]
#cores=cores[cores$Ecosystem!="S",]
##Remove species that don't have individuals in the core
colnames(cores)
cores=cores[,c(1:12,15:16,18,20,22,24,26:27,30,32,35,37,39:40,42:66,68,70:76,78:83,85:92,94:103,106,108,111,113:115,117:124,126:127,129:133,135:140,143:144,146:147,149:152,155,158)]

indvalsummary=indval(cores[,8:120], clustering=cores$Habitat)
summary(indvalsummary, p=0.05,type='short', digits=2, show=p, sort=FALSE)


###Let's try this again with the entire forest. For shits and giggs.
totalcomm=sppabun[,c(1:12,14:20,22,25:30,32:33,35:106,108,110:144,146:147,149:152,154:156,158)]
indvaltotal=indval(totalcomm[,8:147],clustering=sppabun$Habitat)
summary(indvaltotal, p=0.05,type='short', digits=2, show=p, sort=FALSE)

###One last time with reference to edges.
#subset the cores out.
edges=sppabun[sppabun$EP=="Edge"|sppabun$EP=="Trans",]
#remove species that aren't present (ACESIL, CARTOM, QUEBIC, SALNIG, TOXVER, VIBLEN)
edges=edges[,c(1:7,10:12,14:20,22,25:30,33,35:39,41:43,45:48,51:58,60,62:77,79:102,104:105,110,112:113,115:119,122:129,131:132,134:142,144,146:147,150,152,154,156)]
#run the stats
indvalsummary=indval(edges[,8:121], clustering=edges$Habitat)
summary(indvalsummary, p=0.05,type='short', digits=2, show=p, sort=FALSE)
```


```{r CCA}
## Date 1
##Let's take out the TerrTypes that we don't test (like things that include D and thus don't have replicates)
date1.cca=date1[date1$TerrType=="B"|date1$TerrType=="RU"|date1$TerrType=="U"|date1$TerrType=="BU"|date1$TerrType=="UR"|date1$TerrType=="T-UR"|date1$TerrType=="UB"|date1$TerrType=="T-UB"|date1$TerrType=="R"|date1$TerrType=="RB"|date1$TerrType=="BR"|date1$TerrType=="T-RB",]
## the CCA
fly.cca.d1=cca(date1.cca[,8:155]~date1.cca$TerrType)
anova(fly.cca.d1)
plot(fly.cca.d1)

## Date 2
date2.cca=date2[date2$TerrType=="B"|date2$TerrType=="RU"|date2$TerrType=="U"|date2$TerrType=="BU"|date2$TerrType=="UR"|date2$TerrType=="T-UR"|date2$TerrType=="UB"|date2$TerrType=="T-UB"|date2$TerrType=="R"|date2$TerrType=="RB"|date2$TerrType=="BR"|date2$TerrType=="T-RB",]

fly.cca.d2=cca(date2.cca[,8:155]~date2.cca$TerrType)
anova(fly.cca.d2)
plot(fly.cca.d2)

## Date 3
date3.cca=date3[date3$TerrType=="B"|date3$TerrType=="RU"|date3$TerrType=="U"|date3$TerrType=="BU"|date3$TerrType=="UR"|date3$TerrType=="T-UR"|date3$TerrType=="UB"|date3$TerrType=="T-UB"|date3$TerrType=="R"|date3$TerrType=="RB"|date3$TerrType=="BR"|date3$TerrType=="T-RB",]

fly.cca.d3=cca(date3.cca[,8:155]~date3.cca$TerrType)
anova(fly.cca.d3)
plot(fly.cca.d3)

## Date 4
date4.cca=date4[date4$TerrType=="B"|date4$TerrType=="RU"|date4$TerrType=="U"|date4$TerrType=="BU"|date4$TerrType=="UR"|date4$TerrType=="T-UR"|date4$TerrType=="UB"|date4$TerrType=="T-UB"|date4$TerrType=="R"|date4$TerrType=="RB"|date4$TerrType=="BR"|date4$TerrType=="T-RB",]
fly.cca.d4=cca(date4.cca[,8:155]~date4.cca$TerrType)
anova(fly.cca.d4)
plot(fly.cca.d4)
```

```{r Functional Feeding Groups DATE1}
### USING THE CODE FOR SPLITTING TAXONOMIC GROUPS. data.agg will ONLY fit what we specify in the other code. NOTES CAN BE FOUND AT THE END OF THE CODE FOR EACH GROUP

#### All plots
tree.hel.soil=merge(date1.agg,soildata,by="Plot")
tree.hel.soil=merge(tree.hel.soil, tree.hel, by="Plot")

rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,55:89])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$ACERUB+tree.hel.soil$ACESAC+tree.hel.soil$ACESIL+tree.hel.soil$BETALL+tree.hel.soil$CARCAR+tree.hel.soil$CARCOR+tree.hel.soil$CAROVA+tree.hel.soil$CARTOM+tree.hel.soil$CORFLO+tree.hel.soil$CRAPUN+tree.hel.soil$FAGAME+tree.hel.soil$FRAAME+tree.hel.soil$FRANIG+tree.hel.soil$HAMVIR+tree.hel.soil$LIRTUP+tree.hel.soil$MAGACU+tree.hel.soil$MALCOR+tree.hel.soil$NYSSYL+tree.hel.soil$OSTVIR+tree.hel.soil$PLAOCC+tree.hel.soil$POPDEL+tree.hel.soil$POPGRA+tree.hel.soil$PRUSER+tree.hel.soil$QUEALB+tree.hel.soil$QUERUB+tree.hel.soil$SALNIG+tree.hel.soil$SASALB+tree.hel.soil$TILAME+tree.hel.soil$TOXVER+tree.hel.soil$ULMAME+tree.hel.soil$ULMRUB+tree.hel.soil$ULMSP+tree.hel.soil$VIBLEN)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:128])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:12]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:132])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:12]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32+tree.hel.all.pcnm$PCNM33+tree.hel.all.pcnm$PCNM34+tree.hel.all.pcnm$PCNM35+tree.hel.all.pcnm$PCNM36+tree.hel.all.pcnm$PCNM37+tree.hel.all.pcnm$PCNM38)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(96,125,102,98,107)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### CORE+EDGE ####
tree.hel.soil=tree.hel.soil[tree.hel.soil$EP.x=="Core"|tree.hel.soil$EP.x=="Edge",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(23,43,31)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:122])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:12]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:126])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:12]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(91:93,97:98)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## Variance partitioning
varpart(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(23,43,31)], tree.hel.all.pcnm[,c(91:93,97:98)])
## Check to see if it's significant when PCNM and soil come first.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(91:93,97:98)], tree.hel.all.pcnm[,c(23,43,31)])
anova(hel.rda.all.partpcnm)
# Yes.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(23,43,31)], tree.hel.all.pcnm[,c(91:93,97:98)])
anova(hel.rda.all.partsoil)
# Yes.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Core plots ####
tree.hel.soil=tree.hel.soil[tree.hel.soil$EP.x=="Core",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(23,26)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:109])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:12]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:113])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:12]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(113,97,98,111,95,105)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## Variance partitioning
varpart(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(23,26)], tree.hel.all.pcnm[,c(92,95,98,96,108)])
## Check to see if it's significant when PCNM and soil come first.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(92,95,98,96,108)], tree.hel.all.pcnm[,c(23,26)])
anova(hel.rda.all.partpcnm)
# Yes.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(23,26)], tree.hel.all.pcnm[,c(92,95,98,96,108)])
anova(hel.rda.all.partsoil)
# Yes.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```
```{r Functional Feeding Groups DATE2}
### USING THE CODE FOR SPLITTING TAXONOMIC GROUPS. data.agg will ONLY fit what we specify in the other code. NOTES CAN BE FOUND AT THE END OF THE CODE FOR EACH GROUP

#### All plots
tree.hel.soil=merge(date2.agg,soildata,by="Plot")
tree.hel.soil=merge(tree.hel.soil, tree.hel, by="Plot")

rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(42,29,40)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:128])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### CORE+EDGE ####
tree.hel.soil=tree.hel.soil[tree.hel.soil$EP.x=="Core"|tree.hel.soil$EP.x=="Edge",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(29)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:122])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Core plots ####
tree.hel.soil=tree.hel.soil[tree.hel.soil$EP.x=="Core",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:109])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```
```{r Functional Feeding Groups DATE3}
### USING THE CODE FOR SPLITTING TAXONOMIC GROUPS. data.agg will ONLY fit what we specify in the other code. NOTES CAN BE FOUND AT THE END OF THE CODE FOR EACH GROUP

#### All plots
tree.hel.soil=merge(date3.agg,soildata,by="Plot")
tree.hel.soil=merge(tree.hel.soil, tree.hel, by="Plot")

rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(33,37,35)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:127])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:12]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:129])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:12]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32+tree.hel.all.pcnm$PCNM33+tree.hel.all.pcnm$PCNM34+tree.hel.all.pcnm$PCNM35+tree.hel.all.pcnm$PCNM36+tree.hel.all.pcnm$PCNM37)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(103,119:121)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### CORE+EDGE ####
tree.hel.soil=tree.hel.soil[tree.hel.soil$EP.x=="Core"|tree.hel.soil$EP.x=="Edge",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(25,40,44,56)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,52:86])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$ACERUB+tree.hel.soil$ACESAC+tree.hel.soil$ACESIL+tree.hel.soil$BETALL+tree.hel.soil$CARCAR+tree.hel.soil$CARCOR+tree.hel.soil$CAROVA+tree.hel.soil$CARTOM+tree.hel.soil$CORFLO+tree.hel.soil$CRAPUN+tree.hel.soil$FAGAME+tree.hel.soil$FRAAME+tree.hel.soil$FRANIG+tree.hel.soil$HAMVIR+tree.hel.soil$LIRTUP+tree.hel.soil$MAGACU+tree.hel.soil$MALCOR+tree.hel.soil$NYSSYL+tree.hel.soil$OSTVIR+tree.hel.soil$PLAOCC+tree.hel.soil$POPDEL+tree.hel.soil$POPGRA+tree.hel.soil$PRUSER+tree.hel.soil$QUEALB+tree.hel.soil$QUERUB+tree.hel.soil$SALNIG+tree.hel.soil$SASALB+tree.hel.soil$TILAME+tree.hel.soil$TOXVER+tree.hel.soil$ULMAME+tree.hel.soil$ULMRUB+tree.hel.soil$ULMSP+tree.hel.soil$VIBLEN)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:121])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:12]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:123])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:12]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel
##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(96,121,104,116)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

## Variance partitioning
varpart(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(25,40,44,56)], tree.hel.all.pcnm[,c(96,121,104,116)])
## Check to see if it's significant when PCNM and soil come first.
hel.rda.all.partpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(25,40,44,56)], tree.hel.all.pcnm[,c(96,121,104,116)])
anova(hel.rda.all.partpcnm)
# Yes.
hel.rda.all.partsoil=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(96,121,104,116)], tree.hel.all.pcnm[,c(25,40,44,56)])
anova(hel.rda.all.partsoil)
# Yes.

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Core plots ####
tree.hel.soil=tree.hel.soil[tree.hel.soil$EP.x=="Core",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(34,40)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared


###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:109])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:12]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,83:109])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:12]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(91:96,98,101:102)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```
```{r Functional Feeding Groups DATE4}
### USING THE CODE FOR SPLITTING TAXONOMIC GROUPS. data.agg will ONLY fit what we specify in the other code. NOTES CAN BE FOUND AT THE END OF THE CODE FOR EACH GROUP

#### All plots
tree.hel.soil=merge(date4.agg,soildata,by="Plot")
tree.hel.soil=merge(tree.hel.soil, tree.hel, by="Plot")

rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(46,49,45)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:123])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:12]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,83:109])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:12]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30+tree.hel.all.pcnm$PCNM31+tree.hel.all.pcnm$PCNM32+tree.hel.all.pcnm$PCNM33)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(99,114,119,117)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### CORE+EDGE ####
tree.hel.soil=tree.hel.soil[tree.hel.soil$EP.x=="Core"|tree.hel.soil$EP.x=="Edge",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(46,49,43)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:120])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:12]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,83:122])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:12]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19+tree.hel.all.pcnm$PCNM20+tree.hel.all.pcnm$PCNM21+tree.hel.all.pcnm$PCNM22+tree.hel.all.pcnm$PCNM23+tree.hel.all.pcnm$PCNM24+tree.hel.all.pcnm$PCNM25+tree.hel.all.pcnm$PCNM26+tree.hel.all.pcnm$PCNM27+tree.hel.all.pcnm$PCNM28+tree.hel.all.pcnm$PCNM29+tree.hel.all.pcnm$PCNM30)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(113,106,97,115)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

#### Core plots ####
tree.hel.soil=tree.hel.soil[tree.hel.soil$EP.x=="Core",]
dim(tree.hel.soil)
rownames(tree.hel.soil)=tree.hel.soil[,1]
tree.hel.soil=na.exclude(tree.hel.soil)
# Here is the global analysis. Can proceed  Use adj. R square as additional stopping criterion.
colnames(tree.hel.soil)
dim(tree.hel.soil)

hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

##Forward selection
hel.rda.fulsoil.none = rda(tree.hel.soil[,8:12]~1)
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,22:46])

hel.rda.fulsoil.full=rda(tree.hel.soil[,8:12]~tree.hel.soil$Ptot_long+tree.hel.soil$Po_long+tree.hel.soil$pH_long+tree.hel.soil$moist_long+tree.hel.soil$ps1_long+tree.hel.soil$ps2_long+tree.hel.soil$percc_long+tree.hel.soil$cton_long+tree.hel.soil$Ptot_nug+tree.hel.soil$Po_nug+tree.hel.soil$pH_nug+tree.hel.soil$moist_nug+tree.hel.soil$ps1_nug+tree.hel.soil$ps2_nug+tree.hel.soil$percc_nug+tree.hel.soil$pomc_nug+tree.hel.soil$cton_nug+tree.hel.soil$Ptot_short+tree.hel.soil$Po_short+tree.hel.soil$pH_short+tree.hel.soil$moist_short+tree.hel.soil$ps1_short+tree.hel.soil$percc_short+tree.hel.soil$pomc_short+tree.hel.soil$cton_short)

tree.sel=ordiR2step(hel.rda.fulsoil.none,scope=formula(hel.rda.fulsoil.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.fulsoil = rda(tree.hel.soil[,8:12], tree.hel.soil[,c(46,48,43,56,52)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERT BY TREE DATA
hel.rda.fulsoil = rda(tree.hel.soil[,8:12],tree.hel.soil[,c(55:89)])
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared

###INVERTS BY PCMN
#to determine spatial autocorrelation
# First we perform PCNM to get the vectors
colnames(tree.hel.soil)
all.geog.dist=vegdist(tree.hel.soil[,53:54],method="euclid")
all.pcnm1=pcnm(all.geog.dist)
all.pcnmvecs=all.pcnm1$vectors
# If you can get the PCNM library to run, you could do the line below to select only significantly structured PCNMs
#tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs[,which(all.pcnm1$Moran_I[,2]<0.05)])
tree.hel.all.pcnm=cbind(tree.hel.soil,all.pcnmvecs)
names(tree.hel.all.pcnm)
# Now run step 1, global analysis
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,91:109])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared
# now run step 2, the selection part

#forward selection for our PCNM variables
hel.rda.all.pcnm.none=rda(tree.hel.all.pcnm[,8:12]~1)
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,83:111])

hel.rda.all.pcnm.full=rda(tree.hel.all.pcnm[,8:12]~tree.hel.all.pcnm$PCNM1+tree.hel.all.pcnm$PCNM2+tree.hel.all.pcnm$PCNM3+tree.hel.all.pcnm$PCNM4+tree.hel.all.pcnm$PCNM5+tree.hel.all.pcnm$PCNM6+tree.hel.all.pcnm$PCNM7+tree.hel.all.pcnm$PCNM8+tree.hel.all.pcnm$PCNM9+tree.hel.all.pcnm$PCNM10+tree.hel.all.pcnm$PCNM11+tree.hel.all.pcnm$PCNM12+tree.hel.all.pcnm$PCNM13+tree.hel.all.pcnm$PCNM14+tree.hel.all.pcnm$PCNM15+tree.hel.all.pcnm$PCNM16+tree.hel.all.pcnm$PCNM17+tree.hel.all.pcnm$PCNM18+tree.hel.all.pcnm$PCNM19)

tree.sel=ordiR2step(hel.rda.all.pcnm.none,scope=formula(hel.rda.all.pcnm.full),Pin=0.05,R2scope=TRUE)
tree.sel

##Just the variables forward selected
hel.rda.all.fulpcnm=rda(tree.hel.all.pcnm[,8:12], tree.hel.all.pcnm[,c(101,93,98)])
anova(hel.rda.all.fulpcnm)
RsquareAdj(hel.rda.all.fulpcnm)
global.thresh=RsquareAdj(hel.rda.all.fulpcnm)$adj.r.squared

##INVERTS BY ECOSYSTEM
hel.rda.fulsoil = rda(tree.hel.soil[,8:12]~tree.hel.soil$Habitat)
anova(hel.rda.fulsoil)
RsquareAdj(hel.rda.fulsoil)
global.thresh=RsquareAdj(hel.rda.fulsoil)$adj.r.squared
```
-Decomposers never decoupled. No habitat effects.
-Fungi showed sig habitat effects on Date 4 core. No decoupling
-Herbivores had lot of significance in date 3 that we need to expand out, but I'm unwilling to do today. There was only a habitat effect on the Date 3.
-Leaves no signs of decoupling, no significant habitat effects.
-Pollinators no decoupling patterns, habitat effects on date 4.
-Parasites decoupled from soil on date 3 (soil effect in core, not in core+edge or all); sig habitat effect on date 3. No other significances.
-Predators no decoupling. Significant habitat effect in the cores on Dates 1, 2, 4.
-Sap feeders show PCNM effects on date 1 - need to do forward selection still. Significant habitat effects in all locations on dates 1,2,4 but not anywhere in 3.
-Scavengers had no significance anywhere on the first 3 dates and then had a bunch of significance on the fourth date that needs to be looked at more.
-Sugars had the occassional significance to pcnm on date 3 in the middle, but nothing that shows decoupling. No significant habitat effects.
-Woodfeeding organisms showed no significance anywhere.