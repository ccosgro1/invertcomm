...
otu.tax=otu.tax[otu.tax$order=="Diptera",]
commdata=dip
...
level=4
levels(otu.tax[,level])
types=levels(otu.tax[,level])
q=length(types)
n=length(commdata[,1])
...
group="Diptera"
data.nematoda=commdata[,otu.tax[,level]==group]
dim(data.nematoda)
...
date4=data.agg[data.agg$Date=="10.15.16",]
date2=data.agg[data.agg$Date=="5.5.16",]
date3=data.agg[data.agg$Date=="8.10.16",]
date1=data.agg[data.agg$Date=="10.6.15",]
